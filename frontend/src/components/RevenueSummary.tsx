...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId = 'prop-001', debugTenant, showRaw }) => {
    ...
    const formatDecimalForDisplay = (decimalValue: string): string => {
        const trimmed = decimalValue.trim();
        ...
        const paddedFractional = `${fractionalRaw}00`.slice(0, 2);

        return `${negative ? '-' : ''}${groupedWhole}.${paddedFractional}`;
    };

    const displayTotal = data.total_revenue_display ?? data.total_revenue;
                    ...
                    <div>
                        ...
                        <div className="flex items-baseline gap-2 mt-1">
                            <span className="text-3xl font-bold text-gray-900 tracking-tight">
                                {data.currency} {formatDecimalForDisplay(displayTotal)}
                            </span>
